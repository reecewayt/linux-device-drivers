/************************************** e1000-driver ***********************************************************\
 *  \File: e1000-driver.c is an implementation of a PCI Ethernet device driver. The device controls
 *  the LED on the e1000 NIC card by blinking on a kernel timer. User space program can manipulate 
 *  and change the LED status by writing to the device file in /dev. Specifically, users can change to
 *  LED duty cycle
 * 
 *  \Date: 5/29/2024
 * 
...
#include "my-e1000-drv-timer.h"

// Global device structure
struct my_device mydev;
static int blink_rate = -1;  //uninitialized to start

module_param(blink_rate, int, 0660); //stored in /sys/module/$(driver-name)/parameters/blink_rate

// Initialize dev structure
static struct pci_driver e1000_driver = {
    .name = "my_e1000-drv-timer",
    .id_table = pci_ids,
    .probe = e1000_probe,
    .remove = e1000_remove,
};
...
ssize_t e1000_read(struct file *filp, char __user *buffer, size_t len, loff_t *offset) {
    if (copy_to_user(buffer, &blink_rate, sizeof(blink_rate))) {
        return -EFAULT;
    }
    return len;
}
...
ssize_t e1000_write(struct file *filp, const char __user *buffer, size_t len, loff_t *offset) {
    ...
    if(len < sizeof(new_blink_rate)) {
        printk(KERN_ERR "Buffer size is less than %zu bytes\n", sizeof(new_blink_rate)); 
        return -EFAULT; 
    }
    if(copy_from_user(&new_blink_rate, buffer, sizeof(new_blink_rate))) {
        printk(KERN_ERR "Failed to copy from user...\n"); 
        return -EFAULT; 
    }
    if(new_blink_rate > 0) {
        blink_rate = new_blink_rate; 
        mod_timer(&mydev.blink_timer, jiffies + ((HZ / blink_rate) * DEFAULT_DUTY_CYCLE));
        printk(KERN_INFO "New blink rate of %d\n", blink_rate); 
    }
    else if(new_blink_rate == 0){
        printk(KERN_ERR "You enter a zero value for blink rate, please try again...\n");
        return -EINVAL;
    }
    else {
        printk(KERN_ERR "Invalid blink rate entered...\n"); 
        return -EINVAL; 
    }
...
int e1000_open(struct inode *inode, struct file *filp) {
    if(mydev.open_count == 0) { //first time opening /dev device node
        unsigned long next_jiffies;
        mydev.open_count++;
        printk(KERN_INFO "First time opening device file, starting blink timer...\n");

        iowrite32(E1000_LEDCTL_MODE_LED_ON << E1000_LEDCTL_LED0_SHIFT, mydev.hw_addr + E1000_LEDCTL);
        //starts timer interrupts
        next_jiffies = jiffies + ((HZ / blink_rate) * DEFAULT_DUTY_CYCLE);
        mod_timer(&mydev.blink_timer, next_jiffies);
        /* (HZ / blink_rate) * DEFAULT_DUTY_CYCLE: This calculates the duty cycle
         * duration (the time the LED should be on) in ticks.
         * - Example: If the period is 50 ticks and the duty cycle is 0.5, the on-time is 25 ticks.
         */
    }
    else {
        mydev.open_count++;
    ...
    printk(KERN_INFO "%d is the current count of open fd for this driver: %s", mydev.open_count, DEVICE_NAME); 
...
int e1000_release(struct inode *inode, struct file *filp) {
    ...
    if(mydev.open_count == 0){
        printk(KERN_INFO "Last file descriptor closed, stopping blink timer...\n");
        del_timer(&mydev.blink_timer);
         //turn off all leds
        iowrite32(E1000_LEDCTL_MODE_ALL_LED_OFF, mydev.hw_addr + E1000_LEDCTL);
    }
...
void led_timer_callback(struct timer_list *data) {
    ...
    led_status = led_status & E1000_LEDCTL_MODE_MASK;

    // if, LED is on, turn led off -> else, LED is off, turn led on
    (led_status == E1000_LEDCTL_MODE_LED_ON) ? 
        iowrite32(E1000_LEDCTL_MODE_LED_OFF << E1000_LEDCTL_LED0_SHIFT, mydev.hw_addr + E1000_LEDCTL) :
        iowrite32(E1000_LEDCTL_MODE_LED_ON << E1000_LEDCTL_LED0_SHIFT, mydev.hw_addr + E1000_LEDCTL);


    // Schedule next interupt
    next_jiffies = jiffies + ((HZ / blink_rate) * DEFAULT_DUTY_CYCLE);
    mod_timer(&mydev.blink_timer, next_jiffies);
}
...
static int __init e1000_init(void) {
    ...
    // Setup timer to call function callback
    timer_setup(&mydev.blink_timer, led_timer_callback, 0); 

    if (blink_rate > 0)
        printk(KERN_INFO "New default blink rate of %d\n was entered...", blink_rate);
    else {
        blink_rate = DEFAULT_BLINK_RATE;
        printk(KERN_INFO "Using default blink rate of %d\n", blink_rate);
    }

    //initialize device with all LEDs OFF
    iowrite32(E1000_LEDCTL_MODE_ALL_LED_OFF, mydev.hw_addr + E1000_LEDCTL);
